...
/// An implementation of [AMapFlutterPlatform] that uses [MethodChannel] to communicate with the native code.
///
/// The `amap_map` plugin code itself never talks to the native code directly. It delegates
/// all those calls to an instance of a class that extends the AMapFlutterPlatform.
///
...
class MethodChannelAMapFlutter extends AMapFlutterPlatform {
  ...
  MethodChannel ensureChannelInitialized(int mapId) {
    ...
    if (channel == null) {
      channel = MethodChannel('plugins.amap/amap_map_$mapId');
  ...
  /// Set [AMapFlutterPlatform] to use [AndroidViewSurface] to build the Google Maps widget.
  ///
